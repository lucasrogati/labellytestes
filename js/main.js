...
function montarCategoriasDestaque(){
  ...
  wrap.innerHTML = categorias.map((cat, i) => {
    ...
    const categoriaRemota = (window.CATEGORIAS_REMOTAS || []).find(c => c.nome === cat);
    const imagem = categoriaRemota?.imagem_capa || (produtoRef ? produtoRef.imagem : '');
    const descricao = categoriaRemota?.descricao || (produtoRef ? produtoRef.descricao : '');
    return `
...
document.addEventListener('DOMContentLoaded', async () => {
  const grid = document.getElementById('grid');
  if (grid) grid.innerHTML = '<div class="empty-state">Carregando catálogo…</div>';
